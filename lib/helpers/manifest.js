...
function Manifest (ofilename) {
  var site = globals.site();
  var outputfilename = ofilename || 'release.json';
  ...
  function load (remoteData, includeMaster) {
    Object.keys(remoteData).forEach(function (key) {
      if (key !== 'master' || includeMaster) {
  ...
  function readLocalFile (onComplete, includeMaster) {
    fs.readFile(globals.workingDir() + '/dist/' + globals.site() + '/release.json', 'utf8', function (err, data) {
      if (err) {
        ...
        load(JSON.parse(data), includeMaster);
        onComplete();
      }
    });
  }
  ...
  function write () {
    ...
    var fs = require('fs');
    fs.writeFile(globals.dist() + outputfilename, jsonStr, function (err) {
      if (err) {
        throw err;
      }
    });
